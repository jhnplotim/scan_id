...
import com.jcode.scanid.util.CardHelper;
...
public class MainActivity extends Activity implements RecognizerRunnerFragment.ScanningOverlayBinder {
	...
	private BarcodeOverlayController createRecognizerAndOverlay() {
		...
		mBarcodeRecognizer.setScanQrCode(false);
		mBarcodeRecognizer.setScanAztecCode(false);
		mBarcodeRecognizer.setScanCode39(false);
		mBarcodeRecognizer.setScanCode128(false);
		mBarcodeRecognizer.setScanDataMatrix(false);
		mBarcodeRecognizer.setScanEan8(false);
		mBarcodeRecognizer.setScanEan13(false);
		mBarcodeRecognizer.setScanItf(false);
		mBarcodeRecognizer.setScanInverse(false);
		mBarcodeRecognizer.setScanUncertain(false);
		mBarcodeRecognizer.setScanUpca(false);
		mBarcodeRecognizer.setScanUpce(false);
...
//		}
		//TODO: Return error if the returned results don't match UG ID card
		retrieveResult(result.getStringData());
	}

	private void retrieveResult(String stringData) {
		CardData data = null;
		try {
			data = CardHelper.extractCardData(stringData);
		} catch (Exception e) {
			e.printStackTrace();
		}
